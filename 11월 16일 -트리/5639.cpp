...
void postorder(int left, int right, vector<int>& nodes) {//후위 순회 
    if (left > right) //기저조건 : 왼쪽 경계가 오른쪽 경계 너머에 있음
        return; //종료

    //처음으로 root 노드보다 값이 커지는 위치
    int idx = right + 1; 
    for (int i = left; i <= right; i++) { //값 탐색
        if (nodes[i] > nodes[left]) { //left보다 크면  
            idx = i; //idx 변경
            break;//종료
        }
    }
...
int main() {
    int input; //숫자 입력
    vector<int> nodes; //트리 저장

    //입력
    while (cin >> input)
        nodes.push_back(input); //값 트리에 저장
