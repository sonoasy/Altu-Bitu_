...
//우, 상, 좌, 하
int dy[4] = { 0, -1, 0, 1 }; //x방향
int dx[4] = { 1, 0, -1, 0 };//y방향

//크기가 1x1인 정사각형의 네 꼭짓점이 모두 드래곤 커브의 일부인 것이 개수 구하기
int cntSquare(vector<vector<bool>>& board) {
    int ans = 0;
    for (int i = 0; i < SIZE - 1; i++) {//격자 내의 x열 i
        for (int j = 0; j < SIZE - 1; j++) {//격자 내의 y행 j
            if (board[i][j] && board[i][j + 1] && board[i + 1][j] && board[i + 1][j + 1]) //4점의 board 값이 모두 true면
                ans++; //+1
        }
    }
    return ans;//ans 반환
...
ci makeDragonCurve(vector<vector<bool>>& board, vector<int>& direc, ci point) {//드래곤 커브
    int size = direc.size(); //그 전 세대까지의 방향 정보
    for (int i = size - 1; i >= 0; i--) {//size만큼 실행
        int new_direc = (direc[i] + 1) % 4; //현재 시작 점의 방향
        point = { point.first + dy[new_direc], point.second + dx[new_direc] }; //현재 끝 점
        board[point.first][point.second] = true;//board에 기록
        direc.push_back(new_direc);//다음 세대를 위해서 새로운 방향 저장
    }
...
int main() {
    int n, x, y, d, g; //드래곤 커브의 갯수 n,시작점(x,y), 시작방향 d, 세대 g
    ...
    while (n--) {//n만큼 시행
        ...
        while (g--) { //각 세대마다 드래곤 커브 만들기
            end_point = makeDragonCurve(board, direc, end_point);//드래곤커브 시행하고 끝점
        }
    }
