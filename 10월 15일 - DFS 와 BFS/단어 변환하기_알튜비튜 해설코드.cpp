...
//알튜비튜 해설코드 
//생각 못한 부분: begin을 연결관계에 다같이 넣어서 words간 연결관계 list로 생성, 어차피 이미 간곳은 다시 탐색 안하므로 begin간 연결관계 신경쓸필요x , 리스트에 연결관계만 넣어줌 -> q부분 좀더 깔끔해짐
//이미 방문한 표시를 vector가 아닌 map으로 설정 
//words내 target이 있는지 확인하는것을 생각했는데 begin을 연결관계속에 넣어서 큐에 넣다가 도달 안되는식으로도 됨
